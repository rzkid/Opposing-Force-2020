...
void CSnark::Precache(void)
{
	...
	PrecacheModel("models/snark_02.mdl");
...
Class_T	CSnark::Classify(void)
{
	...
	if (GetEnemy() != NULL)
	{
		m_iMyClass = CLASS_ALIEN_BIOWEAPON; // no one cares about it
		...
		case CLASS_COMBINE:
			m_iMyClass = CLASS_NONE;
			return CLASS_ALIEN_BIOWEAPON; // barney's get mad, grunts get mad at it
		}
		m_iMyClass = CLASS_NONE;
	}
...
void CSnark::HuntThink(void)
{
	...
	{
		CPASAttenuationFilter filter(this);
		EmitSound(filter, entindex(), "NPC_Snark.Squeak");
...
void CSnark::SuperBounceTouch(CBaseEntity *pOther)
{
	...
	if (pOther->m_takedamage && m_flNextAttack < gpGlobals->curtime)
	{
		...
		if (tr.m_pEnt == pOther)
		{
			...
			if (tr.m_pEnt->GetModelIndex() != GetModelIndex())
			{
				...
				if (GetParametersForSound("NPC_Snark.Deploy", params, NULL))
				{
	...
	if (!(GetFlags() & FL_ONGROUND))
	{
		...
		CSoundParameters params;
		if (GetParametersForSound("NPC_Snark.Bounce", params, NULL))
		{
			EmitSound_t ep(params);
			ep.m_nPitch = (int)flpitch;
...
{
	return CAI_BaseNPC::IsValidEnemy(pEnemy);
}
