...
void CWeaponRPG::PrimaryAttack( void )
{
	...
	if ( m_hMissile != NULL && m_bGuiding == true)
		return;
	...
	if ( tr.fraction == 1.0 && m_hMissile != NULL)
	{
...
void CWeaponRPG::SuppressGuiding( bool state )
{
	...
	if ( m_hLaserDot == NULL)
	{
		//DevMsg("start guiding in SuppressGuiding()\n");
		//StartGuiding();
...
void CWeaponRPG::ToggleGuiding( void )
{
	...
	{
		//DevMsg("start guiding in ToggleGuiding()\n");
		StartGuiding();
	}
}
