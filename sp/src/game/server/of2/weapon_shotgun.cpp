...
public:
	...
	Vector GetShotgunBulletSpread( void );
	/*
	{
		Vector vitalAllyCone = VECTOR_CONE_3DEGREES;
		float spread = sin( DEG2RAD( dev_shotgun_vector_cone.GetFloat() /2.0f ));
		...
		return Vector(spread,spread,spread);//cone;
	}
	*/
...
void CWeaponShotgun::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
	...
	pOperator->FireBullets( 8, vecShootOrigin, vecShootDir, GetShotgunBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0 );
}
...
void CWeaponShotgun::PrimaryAttack( void )
{
	...
	//Vector DirVecToTarget = vecAiming;
	//VectorNormalize( DirVecToTarget );
	//QAngle angAiming;
	//VectorAngles( DirVecToTarget, angAiming );

	// dont forget to make convars
 ...
 //   	vecAiming = manipulator.ApplySpread( cone, 1.0f );

	//	ShotgunFlechetteCreate( vecSrc,  
	//							vecAiming * 2000.0f, 
	//							3.0f,
	//							128.0f,
	//							2.5f,
	...
	//	NDebugOverlay::Cross3D( vecSrc + (vecAiming * 100.0f), Vector(2,2,2), -Vector(2,2,2), 0, 0, 255, true, 5.0f );
	//}
...
void CWeaponShotgun::SecondaryAttack( void )
{
	...
	// Fire the bullets
	pPlayer->FireBullets( 12, vecSrc, vecAiming, GetShotgunBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0, -1, -1, 0, NULL, false, false );
...
Vector CWeaponShotgun::GetShotgunBulletSpread( void )
{
	Vector vitalAllyCone = VECTOR_CONE_3DEGREES;
	float spread = sin( DEG2RAD( dev_shotgun_vector_cone.GetFloat() /2.0f ));
