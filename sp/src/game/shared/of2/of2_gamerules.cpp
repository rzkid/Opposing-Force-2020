...
//
// Purpose: The Opposing-Force 2 game rules, such as the relationship tables and ammo
//			damage cvars.
//
//=============================================================================
...
LINK_ENTITY_TO_CLASS( of2_gamerules, COpposingForce2Proxy );
IMPLEMENT_NETWORKCLASS_ALIASED( OpposingForce2Proxy, DT_OpposingForce2Proxy )


#ifdef CLIENT_DLL
	void RecvProxy_OF2GameRules( const RecvProp *pProp, void **pOut, void *pData, int objectID )
	{
		COpposingForce2 *pRules = OF2GameRules();
		Assert( pRules );
		*pOut = pRules;
	}

	BEGIN_RECV_TABLE( COpposingForce2Proxy, DT_OpposingForce2Proxy )
		RecvPropDataTable( "of2_gamerules_data", 0, 0, &REFERENCE_RECV_TABLE( DT_OF2GameRules ), RecvProxy_OF2GameRules )
	END_RECV_TABLE()
#else
	void* SendProxy_OF2GameRules( const SendProp *pProp, const void *pStructBase, const void *pData, CSendProxyRecipients *pRecipients, int objectID )
	{
		COpposingForce2 *pRules = OF2GameRules();
		Assert( pRules );
	...
	BEGIN_SEND_TABLE( COpposingForce2Proxy, DT_OpposingForce2Proxy )
		SendPropDataTable( "of2_gamerules_data", 0, &REFERENCE_SEND_TABLE( DT_OF2GameRules ), SendProxy_OF2GameRules )
	END_SEND_TABLE()
#endif
...
//-----------------------------------------------------------------------------
int COpposingForce2::Damage_GetTimeBased( void )
{
#ifdef HL2_EPISODIC
...
// Output :		bool
//-----------------------------------------------------------------------------
bool COpposingForce2::Damage_IsTimeBased( int iDmgType )
{
...
ConVar  alyx_darkness_force( "alyx_darkness_force", "0", FCVAR_CHEAT | FCVAR_REPLICATED );
	...
	//-----------------------------------------------------------------------------
	bool COpposingForce2::ClientCommand( CBaseEntity *pEdict, const CCommand &args )
	{
	...
	//-----------------------------------------------------------------------------
	void COpposingForce2::PlayerSpawn( CBasePlayer *pPlayer )
	{
	}
	...
	// Output  :
	//------------------------------------------------------------------------------
	void COpposingForce2::InitDefaultAIRelationships( void )
	{
		int i, j;
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_NONE,				CLASS_MORTAR_SYNTH,		D_NU, 0);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_PLAYER,			CLASS_MORTAR_SYNTH,		D_HT, 0);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_PLAYER_ALLY,			CLASS_MORTAR_SYNTH,		D_HT, 0);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_PLAYER_ALLY_VITAL,	CLASS_MORTAR_SYNTH,		D_HT, 0);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_MORTAR_SYNTH, CLASS_ALIEN_BIOWEAPON, D_NU, 0);

		// ------------------------------------------------------------
		//	> CLASS_ALIEN_BIOWEAPON (probably only snarks)
		// ------------------------------------------------------------
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_ALIEN_BIOWEAPON, CLASS_MORTAR_SYNTH, D_NU, 0);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_ZOMBIE,			CLASS_MORTAR_SYNTH,		D_HT, 0);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_EARTH_FAUNA,			CLASS_MORTAR_SYNTH,		D_NU, 0);
	...
	// Output  :
	//------------------------------------------------------------------------------
	const char* COpposingForce2::AIClassText(int classType)
	{
		switch (classType)
		{
	...
	void COpposingForce2::PlayerThink( CBasePlayer *pPlayer )
	{
	}

	void COpposingForce2::Think( void )
	{
		BaseClass::Think();
	...
	//-----------------------------------------------------------------------------
	float COpposingForce2::GetAmmoDamage( CBaseEntity *pAttacker, CBaseEntity *pVictim, int nAmmoType )
	{
		float flDamage = 0.0f;
   	...
   	//-----------------------------------------------------------------------------
  	//-----------------------------------------------------------------------------
 	bool COpposingForce2::AllowDamage( CBaseEntity *pVictim, const CTakeDamageInfo &info )
  	{
#ifndef CLIENT_DLL
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool COpposingForce2::NPC_ShouldDropHealth( CBasePlayer *pRecipient )
	{
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool COpposingForce2::NPC_ShouldDropGrenade( CBasePlayer *pRecipient )
	{
		// Can only do this every so often
	...
	//-----------------------------------------------------------------------------
	void COpposingForce2::NPC_DroppedHealth( void )
	{
	...
	//-----------------------------------------------------------------------------
	void COpposingForce2::NPC_DroppedGrenade( void )
	{
...
// ------------------------------------------------------------------------------------ //
// Shared COpposingForce2 implementation.
// ------------------------------------------------------------------------------------ //
bool COpposingForce2::ShouldCollide( int collisionGroup0, int collisionGroup1 )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void COpposingForce2::AdjustPlayerDamageTaken( CTakeDamageInfo *pInfo )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
float COpposingForce2::AdjustPlayerDamageInflicted( float damage )
{
	switch( GetSkillLevel() ) 
	{
	case SKILL_EASY:
...
//---------------------------------------------------------
//---------------------------------------------------------
bool COpposingForce2::ShouldUseRobustRadiusDamage(CBaseEntity *pEntity)
{
#ifdef CLIENT_DLL
	return false;
#endif
...
//---------------------------------------------------------
//---------------------------------------------------------
bool COpposingForce2::ShouldAutoAim( CBasePlayer *pPlayer, edict_t *target )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
float COpposingForce2::GetAutoAimScale( CBasePlayer *pPlayer )
{
#ifdef _X360
	return 1.0f;
#else
...
//---------------------------------------------------------
//---------------------------------------------------------
float COpposingForce2::GetAmmoQuantityScale( int iAmmoIndex )
{
	switch( GetSkillLevel() )
	{
	case SKILL_EASY:
...
void COpposingForce2::LevelInitPreEntity()
{
...
//-----------------------------------------------------------------------------
bool COpposingForce2::IsAlyxInDarknessMode()
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
bool COpposingForce2::ShouldBurningPropsEmitLight()
{
#ifdef HL2_EPISODIC
